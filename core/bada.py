...
from .utils.enums import APSpeedMode, EngineType, Config, FlightPhase, VerticalMode
...
class Bada:
    ...
    def __init__(self, icao, mass_class=2):
        """
        ...
        SYNONYM = np.genfromtxt(Path(__file__).parent.resolve().joinpath('./performance/BADA/SYNONYM.NEW'), delimiter=[3, 2, 7, 20, 25, 8, 5], names=[
        ...
        row = np.where(SYNONYM['ACCODE'] == icao)[0][0]  # Get row index
        file_name = SYNONYM[row][5]
        if(not file_name):
            raise Exception("No aircraft in SYNONYM.NEW")

        # Get data from Operations Performance File (Section 6.4)
        OPF = np.genfromtxt(Path(__file__).parent.resolve().joinpath('./performance/BADA/', file_name+'.OPF'), delimiter=[
        ...
        OPF_Actype = np.genfromtxt(Path(__file__).parent.resolve().joinpath(
            './performance/BADA/', file_name+'.OPF'), delimiter=[5, 15, 1, 12, 26, 1], dtype="U2,U6,i1,U7,U9,U1", comments="CC", autostrip=True, max_rows=1)
        ...
        APF = np.genfromtxt(Path(__file__).parent.resolve().joinpath('./performance/BADA/', file_name+'.APF'), delimiter=[
        ...
        # Aircraft type
        self.__n_eng = OPF_Actype.item()[2]
        """Number of engines"""
        self.__engine_type = {'Jet': 1, 'Turboprop': 2, 'Piston': 3}.get(OPF_Actype.item()[4])
        """engine type [Engine_type enum]"""
        self.__wake_category = OPF_Actype.item()[5]
        """wake category [Wake_category enum]"""

        # Mass
        self.__m_ref = OPF[0][3]
        """reference mass [tones]"""
        self.m_min = OPF[0][4]
        """minimum mass [tones]"""
        self.__m_max = OPF[0][5]
        """maximum mass [tones]"""
        self.__m_pyld = OPF[0][6]
        ...
        if Path(__file__).parent.resolve().joinpath('./performance/BADA/BADA.GPF').is_file():
            GPF = np.genfromtxt(Path(__file__).parent.resolve().joinpath('./performance/BADA/BADA.GPF'),
    ...
    def cal_fuel_burn(self, flight_phase, tas, thrust, alt):
        ...
        # )
        return self.__c_f1 * (1.0 + tas / self.__c_f2) * thrust / 60000
    ...
    def cal_thrust(self, vertical_mode, configuration, H_p, V_tas, d_T, drag, ap_speed_mode):
        ...
        return np.select(
            ...
            choicelist=[
                self.cal_max_climb_to_thrust(H_p, V_tas, d_T),
                ...
                np.minimum(drag, self.__cal_max_cruise_thrust(
                    self.cal_max_climb_to_thrust(H_p, V_tas, d_T))),
                self.__cal_descent_thrust(H_p, self.cal_max_climb_to_thrust(H_p, V_tas, d_T), configuration)])
    ...
    def cal_aerodynamic_drag(self, V_tas, vs, bank_angle, m, rho, configuration, c_des_exp):
        """
        ...
        assert V_tas > 0, "V_tas <= 0"
        assert V_tas >= vs, "V_tas < vs"
        cos_gamma = (V_tas**2 - vs**2)**0.5 / V_tas
        c_L = 2.0 * (m * cos_gamma) * self.__G_0 / (rho * np.square(V_tas) * self.__S * np.cos(np.deg2rad(bank_angle)))
        ...
        # print(f'C_d {c_D}')
        # print(c_D * rho * np.square(V_tas) * self.__S / 2.0 * c_des_exp)
        # Drag force
    ...
    def cas_to_tas(self, V_cas, p, rho):
        ...
        return np.sqrt(2.0/mu * p/rho * (np.power(1.0 + self.__P_0/p * (np.power(1.0 + mu/2.0 * self.__RHO_0/self.__P_0 * np.square(V_cas), 1.0/mu) - 1), mu)-1))
    ...
    def get_norminal_bank_angle(self):
        return self.__PHI_NORM_CIV_OTHERS
    
    def get_g_0(self):
        return self.__G_0
    
    def get_v_stall(self, config):
        """
        Get stall velocity
        ...
        ----------
        configuration: float
            configuration [configuration enum]

        Returns
        -------
        cas stall :float 
            Stall velocity [knots CAS]
        """
        match config:
            case Config.TAKEOFF:
                return self.__v_stall_to
            case Config.INITIAL_CLIMB:
                return self.__v_stall_ic
            case Config.CLEAN:
                return self.__v_stall_cr
            case Config.APPROACH:
                return self.__v_stall_ap
            case Config.LANDING:
                return self.__v_stall_ld
