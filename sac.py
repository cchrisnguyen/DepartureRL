...
from misc import soft_update, hard_update, enable_gradients, disable_gradients, _remote_method
...
class SAC(object):
    ...
    def __init__(self, env, b_rref, p_rref, config):
        ...
        Parameters:
        - env (env object): The environment instance
    ...
    def evaluate(self, env, task_name, write_csv=False):
        self.policy.eval()
        env = env(task_name=task_name)
        s, done= env.reset(write_csv=write_csv), False
        total_noise = 0
        total_fuel = 0
        total_penalty = 0
        total_reward = 0
        while not done:
            # Take deterministic actions during inference
            ...
            total_fuel += info[1]
            total_noise += info[2]
            total_penalty += info[3]

        print(total_reward, total_fuel, total_noise, total_penalty)
